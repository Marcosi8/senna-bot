let handler = async (message, { conn, participants, groupMetadata, args, usedPrefix, text, command }) => {
  ...
  let adminListMessage = `*Lista de Administradores:*\n${listAdmins}\n\n_Envie sua mensagem com confiança, os administradores estão aqui para ajudar!_ 🤝`
  
  try {
    await conn.sendFile(message.chat, pp, 'admins_message.jpg', Buffer.from(formattedMessage + '\n\n' + adminListMessage), message, false, { mentions: groupAdmins.map(v => v.jid) })
  } catch (error) {
    console.error(error)
