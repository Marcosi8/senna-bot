...
const handler = async (m, {text, usedPrefix, command, conn}) => {
  if (!text) throw `️Insira o nome do filme ou série que deseja pesquisar.\nExemplo: ${usedPrefix + command} O Gato com Botas`;
  ...
  if (aaaa == '') throw `️Não foi possível encontrar resultados para "${text}".`;

  const res = await aaaa.map((v) => `*🎬 • Título:* ${v.title}\n*🍿 • Link:* ${v.link}`).join`\n\n───────────────\n\n`;

  const helpMessage = `*💫 • Como usar:*\nUse este comando seguido pelo nome do filme ou série que deseja pesquisar.\nExemplo: ${usedPrefix + command} O Gato com Botas`;

  const ads = `*💫 • Mais um anúncio aqui:*\nhttps://block-this.com/block-this-latest.apk\n\n≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣≣\n\n`;
...
async function searchC(query, numberPage = 1) {
  const $ = await safeLoad(`https://cuevana3.mu/page/${numberPage}/`, { params: {s: query}});
  const resultSearch = [];
