...
const isToxic = /(gandu|puta|maderchod|bhosdike|bhosda|laud?a|chut?iya|maa ki chut|behenchod|behen ki chut|tatto ke saudagar|machar ki jhant|jhant? ka baal|Rand?i ka aulad|chuchi|booob?ie?s|to?lo?l|idiot|nigga|fuck|dick|bitch|tits|bastard|asshole|a[su,w,yu])/i;
...
async function start(client) {
    client.onMessage(async (message) => {
        if (message.body === '!starttoxic') {
            global.db.data.settings['toxicEnabled'] = true;
            await client.reply(message.chatId, 'Toxic detection enabled!', message.id);
        } else if (message.body === '!stoptoxic') {
            global.db.data.settings['toxicEnabled'] = false;
            await client.reply(message.chatId, 'Toxic detection disabled!', message.id);
        }
    });
}
